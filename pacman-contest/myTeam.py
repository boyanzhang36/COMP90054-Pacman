  ...
  """
    ...
    """
        ...
        bestActions = [a for a, v in zip(actions, values) if
                       v == maxValue]  # according to the maxValue choose the best action(s)
...
class BaseAgent(ReflexCaptureAgent):
    ...
    """
        ...
        if len(invadingPacman) > 0:  # the oppenent team has one or two invading pacman
            distToPacman = self.getMazeDistance(myPos, invadingPacman[
                0].getPosition())  # invader[0]: represents the nearest pacman, the distance between me and the nearest pacman
    ...
    def getOppositeGhost(self, gameState):
        ...
        oppositeGhost = [a for a in whoChaseMe if
                         self.getMazeDistance(myPos, a.getPosition()) == min(dists)]  # smallest first
                ...
                for action in actions:
                    ...
                    if self.getMazeDistance(my_nextPos, my_curPos) == 1:
                        if self.getMazeDistance(enemyPosition, my_nextPos) <= 1 and len(list(nextDirection)) == 1:
                            ...
                            priorityQ.push((self.getSuccessor(cur_state[0], action), nextDirection, nextCost + preCost,
                                            preCost, my_nextPos), nextCost + preCost)
                        else:
...
class SecondaryAgent(BaseAgent):
  ...
  """
    ...
    def actionOnDuty(self, gameState, oppositeGhost, myPos, myState, actions):
        if len(oppositeGhost) > 0 and self.getMazeDistance(oppositeGhost[0].getPosition(),
                                                           myPos) <= 2:  # i am the pacman
            myCurrentPos = myState.getPosition()
            ...
            if actToGhost != []:
                ...
                for act in legalActs:
                    ...
                    if not myNextState.isPacman and myState.numCarrying > 0:  # Priority to return
                        return act
                    elif len(originCaps) - len(nextCaps) == 1:  # then eat capsule
                        return act
                    elif len(originFood) - len(nextFood) == 1:  # then eat food
                        return act
                if (len(legalActs) == 0):
                    return self.findFarthestAction(gameState, oppositeGhost[0].getPosition())
                else:
                    ...
                    if len(legalActs) >= 2:
                        for a in legalActs:
                            if self.getSuccessor(gameState, a).getAgentState(self.index).isPacman:
                                actionNow.append(a)
